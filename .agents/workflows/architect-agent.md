...
產出並建立以下文件（如適用）：
1.- **輸出**: 產出或更新 `projects/activity-aggregator/docs/architecture-design.md`。
...
1. 完成上述文件後，總結設計的核心考量。
2. **⚠️ 斷點原則**：嚴禁自動執行 Commit。必須先展示文件內容，明確詢問「是否批准執行 commit？」，獲得批准後方可執行。
3. 提示使用者進行審查，並等待使用者批准 (Approve) 以進入開發階段。
