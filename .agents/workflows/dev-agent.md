...
1. 總結完成的實作項目並展示編譯/測試結果。
2. **⚠️ 斷點原則**：嚴禁與程式碼實作工具同時呼叫 Commit 指令。必須使用 `notify_user` 明確請求 Commit 批准，獲得回覆後單獨執行。
3. **必須在回報末尾明確註明「已通過 [建置指令] 驗證」或「已通過單元測試」。**
4. 提示進入下一個流程（交接給 QA Agent）。
